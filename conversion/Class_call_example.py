...
data_headers = ["stormID","number_of_tstps","timestep","event_date","surge","hm0","period","wDir","dt","sim_year"]
slr = 1.5
# Parse LC Data 
...
for ii in range(0, events.shape[0]):
    ...
    if ii == 0:
        tide_event = obj.get_tidal_prediction('8735180', events['event_date'].values[ii], \
                                              lc_data['event_date'].values[np.array(ii-1)+events['number_of_tstps'].values[ii]], 'MSL')
        tide_df = tide_event
        row_ctr = row_ctr+events['number_of_tstps'].values[ii]-np.array(1)
    else:
        row_ctr = row_ctr+events['number_of_tstps'].values[ii]
        tide_event = obj.get_tidal_prediction('8735180', events['event_date'].values[ii], \
                                        lc_data['event_date'].values[row_ctr], 'MSL')
...
plt.plot(lc_data["timestep"].values[0:72], lc_data["surge"].values[0:72], color='red', label='Surge')
# Plot the second line (Blue)
plt.plot(lc_data["timestep"].values[0:72], tide_df["Prediction"].values[0:72], color='blue', label='Tides')
plt.plot(lc_data["timestep"].values[0:72], surge_w_tides[0:72], color='green', label='surge+tides')
