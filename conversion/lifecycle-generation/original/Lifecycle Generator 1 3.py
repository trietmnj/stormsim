...
def _find_first_greater_index(arr1, arr2):
    results = []
...
for i in range(0, num_LCs + 1):
    ...
    for ii in range(0, lifecycle_duration):
        samples = np.random.poisson(
            lam=lam, size=(ii, 1)
        )  # generate n=lifecycle_duration samples with Lambda=lam
        samples = pd.DataFrame(samples)
        f = np.nonzero(samples)[0]
        ...
        for iii in range(0, len(f)):
            ...
            output_indices = _find_first_greater_index(samples1, Cumulative_probs_a)
            # storm_id =
            ...
            storm_date = pd.DataFrame(
                {"i": f[iii], "year": Yr, "month": Mo, "day": Da, "hour": H}
            )
            ...
            if len(samples1) > 1:
                ...
                while foo:
                    ...
                    output_indices = _find_first_greater_index(
                        samples1, Cumulative_probs_a
                    )
                    Mo = Month[output_indices]
                    Da = Day[output_indices]
                    Hr = np.random.rand(len(samples1[0]), 1)
                    ...
                    storm_date = pd.DataFrame(
                        {"iii": f[iii], "year": Yr, "month": Mo, "day": Da, "hour": H}
                    )
...
for i in range(0, num_LCs + 1):
    ...
    for ii in range(0, lifecycle_duration):
        ...
        for iii in range(0, len(f)):
            ...
            output_indices = _find_first_greater_index(samples1, cdf_a)
