...
class HydroManipulator:
    ...
    def get_tidal_prediction(
        self, station_id, start_date, end_date, datum, save_path=None
    ):
        api_url = (
            ...
            + "&station="
            + station_id
            + "&product=predictions&datum="
            + datum
            + "&time_zone=gmt&interval=h&units=metric&format=csv"
        )
        ...
        if response.status_code == 200:
            ...
            if save_path:
                df.save_csv(save_path, index=False)
            return df
        else:
    ...
    def parse_lc(self, filepath, table_headers):
        ...
        try:
            self.data = pd.read_csv(
                filepath, sep=",", header=None, skip_blank_lines=True
            )
