...
def process_single_storm(hm, storm_id, data, adcirc_h5, wave_h5, group_ids, groups, wave_headers):
    ...
    adcirc_date, adcirc_dt = parse_timestamps(adcirc_date_raw)
    wave_date, wave_dt = parse_timestamps(wave_date_raw)

    # Handle degenerate/missing data case
    if len(adcirc_date) <= 1:
        data["water_elevation"] = np.nan
        data["wave_height"]= np.nan
        data["wave_peak_period"] = np.nan
        data["wave_direction"] = np.nan
        data["date"] = [seed_date]
    ...
    if max_adcirc_dt < max_wave_dt:
        ...
        data["water_elevation"] = yt_elev
        data["wave_height"]= np.array(wave_h5[group_name][wave_headers["Hm0"]])
        data["wave_peak_period"]  = np.array(wave_h5[group_name][wave_headers["Tp"]])
        data["wave_direction"] = np.array(wave_h5[group_name][wave_headers["wDir"]])
        ...
        # ADCIRC Data (masked)
        data["water_elevation"] = np.array(adcirc_h5[group_name]["Water Elevation"])[mask]
        data["wave_height"]= yt_hm0
        data["wave_peak_period"]  = yt_tp
        data["wave_direction"] = yt_wdir
...
#----------------- INPUTS FOR MAIN.PY ----------------------
# Define Input JSON
hydro_config = "../config-files/hydroManipulator_config.json" 
# Define General Path For CHS Metadata Files
chs_meta = "../data/chs-files/regional-files/"
#-----------------------------------------------------------
...
def main():
    ...
    # Read Tidal Config
    if not os.path.exists(hm.config["tide_config"]):
        print(f"Error: Tides json '{hm.config['tide_config']}' not found.")
        sys.exit(1)
    else:
        with open(hm.config['tide_config'], 'r') as f:
    ...
    try:
        ...
        # Returns the actual value (e.g., 29.5) instead of the key name
        sp_lat = np.array(float(next(adcirc_h5.attrs[k] for k in adcirc_h5.attrs if 'latitude' in k.lower() and 'units' not in k.lower()).decode('utf-8')))
        sp_lon = np.array(float(next(adcirc_h5.attrs[k] for k in adcirc_h5.attrs if 'longitude' in k.lower() and 'units' not in k.lower()).decode('utf-8')))
    except Exception as e:
        print(f"Error opening files: {e}")
        sys.exit(1)

    # Pull Grided Dataset
    chs_grid = pd.read_csv(os.path.join(chs_meta, region+"_nodes_metadata.csv"))
    # Find Nearest Data Point To Savepoint
    grd_row = find_nearest_latlon(sp_lat, sp_lon,
                            chs_grid["lat"].to_numpy(), chs_grid["lon"].to_numpy(), max_radius_km=None)[4]
    # Grab Absolute & Relative SSL Bias Values 
    Ba = chs_grid["Ba"].to_numpy()[grd_row]
    Br = chs_grid["Br"].to_numpy()[grd_row]
    depth = chs_grid["depth"].to_numpy()[grd_row]
    ...
    # Get LC Seasonal Adjustment
    season_mask = np.searchsorted(np.array(season_trend['month']), lc_data['month'].to_numpy())
    # Create Tidal Dataset 
    if hm.config["add_tides"]:
        # Define Start Year
        tides_config["start_date"] = str(np.min(lc_data['year'])) + '0101'
        # Define End Year
        tides_config["end_date"] = str(np.max(lc_data['year'])) + '1231'
        # Pull Tidal Signal 
    ...
    try:
        wave_headers, tp_flag = chs_wave_model_header_locator(wave_datasets)
    ...
    for i, storm_data in enumerate(stm_dic):
        ...
        if processed_data:
            ...
            '''
            # -- TC Storm Surge Level Bias Correction
            processed_data["water_elevation"] = hm.correct_bias(processed_data["water_elevation"], Ba, Br)
            # -- Seasonal Steric Water Level Adjustment
            processed_data["water_elevation"] = processed_data["water_elevation"] + season_trend['upper_ci'][season_mask[i]]
            # -- Add Tides
            if hm.config["add_tides"]:
                # Pull Tidal Prediction For Station @ Datum | interval
                tide_time, tide_signal = noaa_py.data_query.filter_tide_data(tidal_ds, processed_data["date"][0], processed_data["date"][-1])
                # Add Tides To Surge
                processed_data["water_elevation"] = hm.add_tides(processed_data["water_elevation"], processed_data["date"], tide_signal, tide_time)
    ...
    if str(hm.config.get("write_single_file", "False")) == "True":
        ...
        write_dicts_to_csv(stm_dic, output_file)
    else:
        ...
        for row in stm_dic:
            ...
            write_dict_to_csv(row, os.path.join(output_dir, filename))
