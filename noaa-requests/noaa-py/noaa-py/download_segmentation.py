def download_segmentation(d_struct, flag1, indx):
    if flag1 != 'm':
        ...
        st_dates = [datetime.datetime.strptime(date, "%Y-%m-%d %H:%M:%S").strftime("%Y%m%d %H:%M") for date in st_dates]
        end_dates = [datetime.datetime.strptime(date, "%Y-%m-%d %H:%M:%S").strftime("%Y%m%d %H:%M") for date in end_dates]
