...
def wl_downloader_v2(id_list, station_list, requested_datum, prod, op_mode, d_beg, d_end):
    if op_mode not in [1, 2]:
        raise ValueError("Please use a valid operational mode (1 - full record, 2 - Specific Date)")
    ...
    id_list = [station_list[valid_indices[i]]['id'] for i, val in enumerate(max_logical) if val]
    ...
    for i, station_id in enumerate(id_list):
        ...
        for product in prod:
            ...
            flag4, flag1, indx = wl_measurements_product_selector_v2(d_struct, product)
            ...
            if op_mode == 2:
                indx, d_end, d_beg, dummy3 = date_search(d_struct, d_beg, d_end, indx)
                if not dummy3:
                    d_struct['start_date'][indx] = f"{d_beg.strftime('%Y-%m-%d %H:%M:%S')} GMT"
                    d_struct['end_date'][indx] = f"{d_end.strftime('%Y-%m-%d %H:%M:%S')} GMT"
                else:
                    s_data_entry.update({
                        'WL_datum': 'Not found',
                        'TP_datum': 'Not found',
            ...
            st_dates, end_dates, st_dates_p, end_dates_p = download_segmentation(d_struct, flag1, indx)

            # Check Datum Availability
            datum, datum_p = datum_selector(d_struct, requested_datum)

            # Check Tidal Predictions Intervals
            interval, flag3 = prediction_interval_selector(d_struct, flag1, d_struct['greatlakes'])
            ...
            s_data = wl_download(ctr, s_data, datum, station_id, timezone, units, data_format, st_dates, end_dates, gen_url, timeout, flag1)

            # Download Tidal Predictions
            s_data = tidal_predictions_downloader(
                ctr, s_data, datum_p, station_id, timezone, units, data_format,
                interval, st_dates_p, end_dates_p, gen_url, timeout, d_struct['greatlakes'], flag1, d_struct
            )
